...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --output-dir)
            shift
            [ "$#" -gt 0 ] || { echo "Ошибка: для --output-dir нужно указать каталог." >&2; exit 2; }
            FSTEK_OUTPUT_DIR_ARG="$1"
            ;;
        --output-dir=*)
            FSTEK_OUTPUT_DIR_ARG="${1#*=}"
            ;;
        --batch-id)
            shift
            [ "$#" -gt 0 ] || { echo "Ошибка: для --batch-id нужно указать идентификатор." >&2; exit 2; }
            FSTEK_BATCH_ID_ARG="$1"
            ;;
        --batch-id=*)
            FSTEK_BATCH_ID_ARG="${1#*=}"
            ;;
...
fstek_run_all_checks() {
    ...
    if [ "$SELECTED_COUNT" -eq 0 ]; then
        ...
        if [ -n "$FSTEK_REPORT_JSON" ]; then
            fstek_report_write_summary_json 2 "FAIL"
        fi
        return 2
    fi
    ...
    if [ -n "$FSTEK_REPORT_JSON" ]; then
        if [ "$TOTAL_FAIL" -eq 0 ] && [ "$RUN_ERROR" -eq 0 ]; then
            fstek_report_write_summary_json 0 "$OVERALL_RESULT"
        else
            fstek_report_write_summary_json 1 "$OVERALL_RESULT"
        fi
    fi
...
if [ -n "$FSTEK_REPORT_LOG" ]; then
    set +e
    fstek_run_all_checks 2>&1 | tee "$FSTEK_REPORT_LOG"
    RUN_EXIT=${PIPESTATUS[0]}
    set -e
    echo ">>> JSON summary: $FSTEK_REPORT_JSON"
    exit "$RUN_EXIT"
fi
