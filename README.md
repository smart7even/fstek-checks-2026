...
При запуске с `--class` выполняются только те усиления, которые указаны для выбранного класса в таблицах `Реализация в информационной системе` методического документа. Старый флаг `--with-enhancements` оставлен для совместимости и включает все реализованные проверки усилений без фильтрации по классу.
