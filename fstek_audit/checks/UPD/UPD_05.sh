#!/usr/bin/env bash
# fstek_audit/checks/UPD/UPD_05.sh - migrated measure logic for УПД.5.
...
run_check() {
    ...
    if $GDM_CONFIGURED; then
        ...
        check_info "УПД.5.4" "Графическая оболочка не используется или предупреждение не настроено"
    fi
